...
{/if}
	...
	<details class="px-0 not-prose" open>
		...
		{#if !data.statusData.error && !data.heartbeatApi.error}
			{#each data.statusData.publicGroupList as category}
				...
				{#each category.monitorList as instance}
					...
					<a href={"https://" + instance.name.split(" ")[itemsInArray === 3 ? 2 : 1].split("(")[1]?.split(")")[0]} class="{isUp ? "" : "pointer-events-none"} no-underline hover:brightness-100">
						<Card name={itemsInArray === 3 ? instance.name.split(" ")[0] + " " + instance.name.split(" ")[1] : instance.name.split(" ")[0]} color={instance.tags[0].color}>
							<div class="flex flex-row flex-wrap gap-1" slot="btn">
								{#if !isUp}
